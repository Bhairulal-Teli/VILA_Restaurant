...
const StyledSalesChart = styled(DashboardBox)`
  ...
  & .recharts-cartesian-grid-vertical line { stroke: var(--color-grey-300); }
`;

interface SalesChartProps { bookings: any[]; numDays: number; }
...
function SalesChart({ bookings, numDays }: SalesChartProps) {
  ...
  const data = allDates.map((date) => ({
    ...
    extrasPrice: (bookings ?? []).filter((b) => isSameDay(date, new Date(b.created_at))).reduce((acc, cur) => acc + cur.extrasPrice, 0),
  }));
    ...
    <StyledSalesChart>
      ...
      <ResponsiveContainer height={300} width="100%">
        <AreaChart data={data}>
          ...
          <Area dataKey="extrasPrice" type="monotone" stroke={colors.extrasSales.stroke} fill={colors.extrasSales.fill} strokeWidth={2} name="Extra Sales" unit="$" />
        </AreaChart>
      </ResponsiveContainer>
    </StyledSalesChart>
  );
